...
import Cardherbi from '../Card/Cardherbi';
...
function Herbicide() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios().get('/user/ViewAllHerbicide'); // Replace with your actual API endpoint
  ...
  return (
    <div>
    <Home />
    <Exclusive value={"Herbicide Products"}/>
    ...
    <div className='flex p-8 flex-row flex-wrap justify-between bg-red-400 bg-zinc-100 w-full'>
      ...
      {data.map((item, index) => (
        <Cardherbi data={item} key={index} index={index} />
      ))}
      </div>
    </div>
  );
