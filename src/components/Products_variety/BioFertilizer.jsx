...
import Cardbiof from '../Card/Cardbiof';
import Exclusive from '../Category/Exclusive';
import Home from '../Home';

function BioFertilizer() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await Axios().get('/user/ViewAllBioFertilizer'); // Replace with your actual API endpoint
  ...
  return (
    <div>
    <Home />
    <Exclusive value={"BioFertilizer Products"}/>
    ...
    <div className='flex p-8 flex-row flex-wrap justify-between bg-red-400 bg-zinc-100 w-full'>
      ...
      {data.map((item, index) => (
        <Cardbiof data={item} key={index} index={index} />
      ))}
      </div>
    </div>
  );
